...
/// `bounds` governs the per-connection resource limits `handler::handle_connection`'s own read
/// loop enforces plus the concurrency/duration caps `run_tcp_listener` enforces directly. `main.rs`
