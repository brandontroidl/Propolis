//! sensor-telnet: the Telnet honeypot sensor binary. Binds one TCP address, negotiates minimal
//! Telnet options, captures a login attempt (accepting any credential), then presents the shared
//! fake shell - see `internal/design/08-remaining-sensors.md`'s "sensor-telnet" section for the
//! protocol flow this binary composes and `handler.rs` for the session logic.
//!
//! Configuration is environment variables (see the `ENV_*` constants below), matching the
//! convention established by `sensor-ssh`/`sensor-catchall`'s own `main.rs`. Every value is
//! validated at startup and the process refuses to start on a malformed one rather than silently
//! substituting a default that could disable the bound it names.
...
const ENV_LOG_PATH: &str = "PROPOLIS_TELNET_LOG_PATH";
const ENV_READ_TIMEOUT_MS: &str = "PROPOLIS_TELNET_READ_TIMEOUT_MS";
const ENV_IDLE_TIMEOUT_MS: &str = "PROPOLIS_TELNET_IDLE_TIMEOUT_MS";
...
enum ConfigError {
    /// `PROPOLIS_TELNET_BIND` was absent or unparseable.
    NoBind,
    InvalidBind(String),
...
/// Resolve the outbox directory: the explicit `PROPOLIS_TELNET_OUTBOX_DIR` override if set, else
/// a subdirectory of the sensor's own resolved spool root. The default must derive from
/// `spool_dir` (not a fixed constant) so it also follows a `PROPOLIS_TELNET_SPOOL_DIR` override,
...
async fn main() {
    ...
    let config = match load_config_from_env() {
        Ok(c) => c,
        Err(e) => {
            tracing::error!(error = %e, "sensor-telnet: invalid configuration; refusing to start");
            std::process::exit(1);
        }
    };
    ...
    .await
    {
        ...
        Err(e) => {
            tracing::error!(
                ...
                error = %e,
                "sensor-telnet: failed to start server"
            );
            std::process::exit(1);
        }
    };
...
mod tests {
    ...
    fn outbox_defaults_under_the_spool_root() {
        // With no PROPOLIS_TELNET_OUTBOX_DIR override, the outbox must sit under the resolved
