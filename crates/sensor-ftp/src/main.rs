...
async fn main() {
    ...
    let wan_map = match parse_wan_map(&env::var(ENV_WAN_MAP).unwrap_or_default()) {
        Ok(m) => m,
        Err(e) => { tracing::error!(error = %e, "sensor-ftp: config error"); std::process::exit(1); }
    };
    let log_path = env::var(ENV_LOG_PATH).map(PathBuf::from).unwrap_or_else(|_| PathBuf::from(DEFAULT_LOG_PATH));
    let spool_dir = env::var(ENV_SPOOL_DIR).map(PathBuf::from).unwrap_or_else(|_| PathBuf::from(DEFAULT_SPOOL_DIR));

    let bounds = match (|| -> Result<ConnectionBounds, ConfigError> {
        Ok(ConnectionBounds {
