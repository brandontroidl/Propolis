...
impl std::fmt::Display for ConfigError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ConfigError::NoBind => write!(f, "{ENV_BIND} must be set to a single ip:port bind address"),
            ConfigError::InvalidBind(s) => write!(f, "invalid {ENV_BIND} address {s:?}"),
            ConfigError::InvalidWanMapEntry(s) => write!(f, "invalid {ENV_WAN_MAP} entry {s:?}, expected local_ip=wan_ip"),
            ConfigError::InvalidBound { field, value } => write!(f, "{field} must be a positive integer, got {value:?} (zero never means unlimited)"),
        }
    }
}
...
fn parse_positive_u64(raw: Option<&str>, default: u64, field: &'static str) -> Result<u64, ConfigError> {
    ...
    if value == 0 {
        return Err(ConfigError::InvalidBound { field, value: raw.to_string() });
    }
    Ok(value)
}
...
fn load_config_from_env() -> Result<Config, ConfigError> {
    ...
    let bind_addr: SocketAddr = bind_raw.trim().parse().map_err(|_| ConfigError::InvalidBind(bind_raw.clone()))?;
...
mod tests {
    ...
    fn parse_wan_map_rejects_malformed() {
        assert!(matches!(parse_wan_map("not-valid"), Err(ConfigError::InvalidWanMapEntry(_))));
    }
    ...
    fn parse_positive_u64_rejects_zero() {
        assert!(matches!(parse_positive_u64(Some("0"), 42, "x"), Err(ConfigError::InvalidBound { .. })));
    }
}
