# Sub-project 1: core scoring spine

Detailed design spec for the Propolis-new core spine (Rust + PostgreSQL). This is the
...
The spine owns four things and nothing else:
...
The spine has no network listeners, no vendor clients, no web surface, and no scheduler. It
...
Deployment context the spine must serve: every WAN-IP collector, whether one multi-homed node
...
The spine computes three derived facts per source IP. They are distinct and must not be
...
- RECOMMENDED. An eligible IP whose breadth-boosted score crosses a tier floor:

  ```
  recommended  =  eligible AND tier is not None
  ```

  A recommended IP is actively surfaced and queued for operator approval by a later
  sub-project. The spine only computes the flag; it never reports or publishes.

INVARIANT (load-bearing, enforced by test): breadth affects weight and recommendation only. It
can never set `has_confirmed_real` and can never make an ineligible IP eligible. Only a
confirmed-real event does that. Rationale: reports built on spoofable UDP or a lone SYN get a
...
The spine is event-sourced. Collectors append immutable events to a hash-chained ledger. The
...
- Advancing the projection is a write path. In single-node mode the process advances it
  ...
  mechanism itself belongs to sub-project 7 (runtime and coordination); the spine only assumes
...
raw payloads are dropped at capture time by the sensor, upstream of the spine, and never reach
...
- `half_life_seconds` default 21600 (6 hours). Recommended and tunable; it is the primary
  scoring knob.
...
Recommended starting values, flagged tunable and unsettled:

- `BREADTH_PER_WAN = 0.15`
- `BREADTH_CAP = 0.60`

With these, one WAN IP gives factor 1.00, and five or more WAN IPs give factor 1.60. The
breadth factor feeds weight and tier only. It is structurally incapable of touching
`has_confirmed_real` or the eligibility legs.
...
Tier is computed on the breadth-boosted effective score. Both axes of a band must clear their
floor. Recommended defaults:

```
AGGRESSIVE  if  effective_score >= 90  AND  max_confidence >= 0.95
STANDARD    if  effective_score >= 75  AND  max_confidence >= 0.70   (tested after AGGRESSIVE)
otherwise   ->  None
```

A score of 92 with confidence 0.80 is STANDARD, not AGGRESSIVE: the confidence floor is not
met. `tier` and `eligible` are independent facts; an IP can be eligible with `tier = None`.
...
The spine is verified with property-based tests and boundary example tests, exercising the real
...
These are the values and design choices the operator must settle. All are currently recommended
defaults, not fixed decisions.

1. Breadth constants. `BREADTH_PER_WAN = 0.15` and `BREADTH_CAP = 0.60` are proposals; the real
   curve and ceiling are unsettled. Related: whether sensor-breadth (`distinct_sensor_count`)
   should also factor into the multiplier, or whether WAN-breadth alone is the right signal.
2. Which score feeds the tier gate. This spec proposes the breadth-boosted effective score.
   The alternative is basing tier on the base `raw_score` and letting breadth influence only
   the recommendation surface. Proposed: boosted.
3. The recommendation threshold. Recommendation is currently tied directly to tier membership
   (`recommended = eligible AND tier is not None`). Whether recommendation should have its own
   independent threshold, separate from the feed tier floors, is open.
4. The half-life default in the multi-WAN context. 21600 seconds (6 hours) is inherited as a
   reasonable single-node value. Whether it remains appropriate when signal aggregates across
   several WAN IPs into one score, which can accumulate faster, needs validation.
