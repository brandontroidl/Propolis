...
2. **Password** → `401 Unauthorized`. The password comes from
   ...
   dropped and only the hash kept in memory. There is no password reset flow —
   change the env var and restart. An empty/absent password means the console
...
- **No global Content-Security-Policy** — the console sets only `X-Frame-Options:
  ...
  CSP is `/samples/download`. So a broken page is not a CSP blocking assets —
  look at the proxy or the network path instead.
